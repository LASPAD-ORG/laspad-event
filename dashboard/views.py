    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and location_form.is_valid():
            ...
            if request.FILES.get('program_pdf'):
                event.program_pdf = request.FILES['program_pdf']
                event.save(update_fields=['program_pdf'])
            elif request.POST.get('program_pdf_clear'):
...
def schedule_manage(request, pk):
    ...
    if request.method == 'POST':
        ...
        if action == 'add_day':
            ...
            day = get_object_or_404(EventDay, pk=request.POST.get('day_pk'), event=event)
            ...
            day = get_object_or_404(EventDay, pk=request.POST.get('day_pk'), event=event)
            day.delete()
            messages.warning(request, "Jour supprimé.")
            ...
            session = get_object_or_404(Session, pk=request.POST.get('session_pk'), day__event=event)
...
def export_registrations_csv(request, pk):
    ...
    for reg in registrations:
        p = reg.participant
        writer.writerow([
            p.first_name, p.last_name, p.email, p.institution, p.role, p.phone,
            reg.get_status_display(), reg.registered_at.strftime('%d/%m/%Y %H:%M'), reg.motivation,
        ])
    return response
...
def export_presence_csv(request, pk):
    ...
    response = HttpResponse(content_type='text/csv; charset=utf-8')
    response['Content-Disposition'] = f'attachment; filename="presences_{event.slug}.csv"'
    response.write('\ufeff')
    writer = csv.writer(response, delimiter=';')
...
def participants_list(request):
    participants = Participant.objects.annotate(event_count=Count('registrations')).order_by('-created_at')
...
def contact_participants(request, pk):
    ...
    if request.method == 'POST':
        ...
        if target == 'all':
            ...
            regs = event.registrations.filter(status=Registration.STATUS_PENDING).select_related('participant')
        else:
            regs = registrations
        ...
        from django.conf import settings
        emails = tuple((subject, body, settings.DEFAULT_FROM_EMAIL, [e]) for e in recipients)
        try:
            send_mass_mail(emails, fail_silently=False)
            messages.success(request, f"Message envoyé à {len(recipients)} participant(s).")
        except Exception as e:
            import logging
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            html_message = render_to_string('emails/invite.html', {
                ...
                'site_name': site_name, 'login_url': f"{site_url}/dashboard/login/",
            })
            send_mail(
                subject=f"[{site_name}] Votre accès administrateur — Bienvenue",
                message=f"Bonjour {user.get_full_name()},\n\nIdentifiant : {username}\nMot de passe : {password}\nConnexion : {site_url}/dashboard/login/",
                from_email=settings.DEFAULT_FROM_EMAIL,
            ...
            import logging
            logging.getLogger(__name__).warning(f"Email invitation non envoyé : {e}")

        messages.success(request, f"{role_label} « {user.get_full_name() or username} » créé. Email envoyé à {email}.")
...
def user_suspend(request, pk):
    ...
    target.save()
    messages.warning(request, f"Compte de « {target.get_full_name() or target.username} » suspendu.")
    return redirect('dashboard:user_list')
...
def user_activate(request, pk):
    ...
    target.save()
    messages.success(request, f"Compte de « {target.get_full_name() or target.username} » réactivé.")
    return redirect('dashboard:user_list')
...
def user_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        target.delete()
        messages.success(request, f"Compte « {name} » supprimé définitivement.")
        return redirect('dashboard:user_list')
...
def organizer_create(request):
    ...
    if request.method == 'POST':
        ...
        organizer.institution = institution; organizer.bio = bio; organizer.phone = phone
...
def scan_home(request):
    events = Event.objects.filter(status=Event.STATUS_PUBLISHED, end_datetime__gte=timezone.now()).order_by('start_datetime')
...
def scan_ticket(request, token):
    ...
    if request.method == 'POST':
        ...
        if action == 'mark_onsite' and not reg.attended_onsite:
            ...
            messages.success(request, f"✅ Présence confirmée — {reg.participant.full_name}")
        elif action == 'mark_online' and not reg.attended_online:
            reg.mark_attended_online()
            messages.success(request, f"✅ Connexion confirmée — {reg.participant.full_name}")
...
def scan_lookup(request):
    ...
    if not reg:
        reg = Registration.objects.filter(participant__email__iexact=q, status=Registration.STATUS_ACCEPTED).first()
    if reg:
